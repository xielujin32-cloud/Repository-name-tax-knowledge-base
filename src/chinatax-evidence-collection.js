...
function titleFromDetail(html) {
  ...
  const pageTitle = textFromFirst(html, /<title\b[^>]*>([\s\S]*?)<\/title>/i);
...
export function parseChinaTaxPolicyEvidence(html) {
  ...
  const documentNo = documentNoFromText(normalizedText);
  return Object.freeze({
    title: titleFromDetail(html),
    document_no: documentNo,
    issuing_authority: authoritiesFromDocumentNo(documentNo),
    publish_date: labelledDate(normalizedText, ['发布日期', '发布时间', '成文日期', '发文日期']),
