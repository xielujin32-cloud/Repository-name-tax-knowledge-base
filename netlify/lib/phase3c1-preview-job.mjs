...
// A passed Job freezes the complete reviewed material set before its terminal
// transition. Safe metadata is stored in the database; raw content remains in
// protected immutable object storage and is never exposed by Job status APIs.
export async function runPhase3C1PreviewJob({ job_id, repository, fetchImpl = fetch, collectPreview = collectPhase3C1ApplyMaterial } = {}) {
  ...
  try {
    ...
    const { preview, materials } = await collectPreview({ fetchImpl, frozen_selection_input: frozenSelection, time_budget_ms: PHASE3C1_ASYNC_PREVIEW_TIME_BUDGET_MS, onProgress: (progress) => repository.updatePhase3C1PreviewJobProgress(job_id, progress) });
    await repository.persistPhase3C1PreviewJobMaterials(job_id, { preview, materials, frozen_selection_input: frozenSelection });
