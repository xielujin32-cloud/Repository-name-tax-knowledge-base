...
import { PHASE3C1_FIXED_IMPORT_URLS, diagnosePhase3C1ImportPreview, preparePhase3C1ImportPreview } from '../src/phase3c1-controlled-import.js';
...
async function request(handler, pathname, { token = '' } = {}) {
  const response = await handler(new Request(`https://taxkb.example${pathname}`, { headers: token ? { authorization: `Bearer ${token}` } : {} }));
